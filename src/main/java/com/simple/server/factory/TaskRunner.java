	...
	public void setParam(ParameterType parameterType, Class clazz, Object object) {
		if (tasks.containsKey(clazz)) {
			...
			for (Task task : taskList) {
				...
				case ON_BEFORE_START_TASK:
					task.setOnBeforeStartTask((CommandType) object);
					break;
				case ON_AFTER_TASK_DONE:
					task.setOnAfterTaskDone((CommandType) object);
					break;
				case DEACTIVATE_AFTER_TASK_DONE:
					task.setDeactivateMySelfAfterTaskDone((Boolean) object);
					break;
				case ON_RUNTIME_ERROR:
					task.setOnRuntimeError((CommandType) object);
					break;
				}
			}
		}
